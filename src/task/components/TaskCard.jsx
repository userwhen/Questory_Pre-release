...
export default function TaskCard({ task, onToggle, onOpenDetail, onToggleSub, onIncrement, isSelectMode, isSelected, onToggleSelect, readOnly, onLongPress, onEdit, onDragStart, onDragMove, onDragEnd, skillIconMap }) {
  ...
  const btnPressTimer = useRef(null); // 拖曳把手在非勾選模式下的長按計時器（長按進勾選模式）
  const btnPressOrigin = useRef({ x: 0, y: 0 });
  ...
  const clearBtnPressTimer = () => {
    if (btnPressTimer.current) { clearTimeout(btnPressTimer.current); btnPressTimer.current = null; }
  };

  const handlePointerDown = (e) => {
    if (readOnly || isSelectMode) return;
    pressOrigin.current = { x: e.clientX, y: e.clientY };
    pressTimer.current = setTimeout(() => onEdit && onEdit(task), LONG_PRESS_MS);
  };
    ...
    <div data-task-id={task.id} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', marginBottom: 10 }}>
      ...
      >
        <div style={cardRowStyle}>
          ...
          {/* 3. 右側：拖曳把手（非勾選模式長按進勾選模式；勾選模式按住即拖曳排序） */}
          {!readOnly && (
            isSelectMode ? (
              <span
              ...
              <span
                onPointerDown={e => {
                  e.stopPropagation();
                  btnPressOrigin.current = { x: e.clientX, y: e.clientY };
                  btnPressTimer.current = setTimeout(() => onLongPress(task.id), LONG_PRESS_MS);
                }}
                onPointerMove={e => {
                  e.stopPropagation();
                  if (!btnPressTimer.current) return;
                  const dx = Math.abs(e.clientX - btnPressOrigin.current.x);
                  const dy = Math.abs(e.clientY - btnPressOrigin.current.y);
                  if (dx > MOVE_CANCEL_PX || dy > MOVE_CANCEL_PX) clearBtnPressTimer();
                }}
                onPointerUp={e => { e.stopPropagation(); clearBtnPressTimer(); }}
                onPointerLeave={e => { e.stopPropagation(); clearBtnPressTimer(); }}
                onPointerCancel={e => { e.stopPropagation(); clearBtnPressTimer(); }}
                style={{ cursor: 'pointer', color: 'var(--text-ghost)', fontSize: '1.1rem', flexShrink: 0, touchAction: 'none', padding: '0 2px', opacity: 0.5 }}
              >☰</span>
            )
          )}
        </div>
