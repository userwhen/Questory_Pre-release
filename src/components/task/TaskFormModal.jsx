...
import { TaskDict, REMINDER_MODES } from '@/data/taskdict.js';
import LocationPickerModal from '@/components/task/LocationPickerModal.jsx';
...
const CATEGORY_VISIBLE_FIELDS = {
  ...
  guild: ['desc', 'subs', 'matrix', 'recurrence', 'deadline', 'reminder', 'location', 'skills'],
};
...
export default function TaskFormModal({ initial, cats, skills = [], unlocks = {}, openAchievements = [], onSave, onDelete, onClose }) {
  ...
  const estimatedRewards = useMemo(() => {
    const w = (form.importance * 1.5) + (form.urgency * 0.5);
  ...
  const CaloriesBlock = (unlocks.feature_cal && form.cat === '運動') ? (
    <div style={{ ...boxStyle, background: 'var(--color-gold-soft)', borderColor: 'var(--color-gold)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
        <input type="number" style={{ ...inputStyle, width: 70, marginBottom: 0, padding: 6, border: 'none', background: 'rgba(255,255,255,0.7)' }} value={form.calories} onChange={e => set('calories', parseInt(e.target.value) || 0)} />
  ...
  const MatrixBlock = (
    <div style={{ ...boxStyle, marginBottom: 15 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>💰{estimatedRewards.gold} ✨{estimatedRewards.exp}</span>
      </div>
      <div>
  ...
  const AchLinkBlock = (
    <div style={{ ...boxStyle, marginBottom: 15 }}>
      ...
      {form.achLink?.mode === 'new' && (
        <input style={inputStyle} placeholder="新成就標題，例如：App 上架計畫" value={form.achLink.title}
  ...
  const CountBlock = (
    <div style={{ ...boxStyle, marginBottom: 15 }}>
      <label style={labelStyle}>{dDict.targetLabel || '重複次數'}</label>
      <input
        type="text"
        inputMode="numeric"
        pattern="[0-9]*"
        ...
        onChange={e => {
          ...
          const v = parseInt(numericValue, 10) || 0;
          setForm(f => ({ ...f, target: v, type: v > 0 ? 'count' : 'normal' }));
        }}
        placeholder="輸入次數 (0=不使用)"
      />
    </div>
  );
    ...
    >
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          ...
          <input style={inputStyle} placeholder="要做什麼呢？" value={form.title} onChange={e => set('title', e.target.value)} />
        </div>
