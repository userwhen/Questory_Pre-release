...
import { AvatarShop, ALL_ITEMS } from '@/avatar/data/avatar_config.js';
import { AvatarStagePets, AvatarPetDoorModals } from '@/avatar/components/AvatarPetPanel.jsx';
...
function AvatarStage({ wearing, activePets }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', pointerEvents: 'none' }}>
      {(() => {
        // 更衣室預覽舞台跟 MainPage 比例完全不同（矮胖 vs 瘦高），
        // 這個 70/30 分界是獨立調的，不用跟 MainPage 的 CHAR_FOOT_RATIO 同步。
        const wallBg = w.wall_bg ?? w.bg;
        const floorBg = w.floor_bg ?? w.bg;
        return (
          <>
            {wallBg && <img src={`img/${wallBg}.png`} style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '70%', objectFit: 'cover', opacity: 0.6, zIndex: 1 }} onError={e => { e.target.style.opacity = '0'; }} alt="" />}
            {floorBg && <img src={`img/${floorBg}.png`} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '30%', objectFit: 'cover', opacity: 0.6, zIndex: 1 }} onError={e => { e.target.style.opacity = '0'; }} alt="" />}
          </>
        );
      })()}
      ...
      <AvatarStagePets pets={activePets} />
    </div>
  );
}
...
export default function AvatarPage({ onNavigate }) {
  ...
  const handleSetMode = useCallback(m => {
    setMode(m);
    const defaults = { avatar: 'suit', pet: 'pet', decor: 'wall_bg' };
  ...
  const MODE_TABS = {
    ...
    pet: ['pet', 'companion'],
    decor: ['wall_bg', 'floor_bg', 'furniture'],
  };
  const TAB_LABELS = {
  ...
  // 只認真實 avatar.wearing / activePets，不用 previewWearing
