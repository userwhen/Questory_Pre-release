...
import { getShopItems as getShopItemsPure, getStackedBag as getStackedBagPure } from '@/shop/utils/shopSelectors.js';
// 精力藥水固定清單：用完整 id 比對，不要用字串包含判斷（換名字或加新藥水時容易漏比對到）
...
export const ShopEngine = {
  ...
  buyItem(id, qty = 1) {
    ...
    setState(store => {
      ...
      // 扣庫存（查表判斷是不是系統商品，不用字串前綴——就算 ID 已經統一 sys_ 開頭，
      // 這裡還是查表以防未來又出現漏加前綴的情況）
      const sysShop = { ...store.sysShop };
      if (this.systemPrototypes.some(p => p.id === id)) {
  ...
  useItem(id, qty = 1) {
    ...
    } else if (item.category === '其他' && STAMINA_ITEM_IDS.includes(id)) {
      setState(store => {
